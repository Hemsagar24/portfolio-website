              ...
              >
                {filteredCertifications.map((cert, index) => (
                  <motion.div
                    ...
                    onHoverStart={() => setHoveredCard(`cert-${index}`)}
                    onHoverEnd={() => setHoveredCard(null)}
                    className="group relative"
                  >
                    <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-7 shadow-2xl border border-slate-200/50 hover:shadow-3xl hover:border-blue-300/50 transition-all duration-700 relative overflow-hidden h-full">
                      {/* Enhanced background effects */}
                      <div className={`absolute inset-0 bg-gradient-to-br ${cert.color} opacity-0 group-hover:opacity-8 transition-opacity duration-700`} />
                      <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-br from-white/10 to-transparent rounded-full transform translate-x-20 -translate-y-20 group-hover:scale-150 transition-transform duration-1000" />
                      ...
                      <div className="relative z-10 h-full flex flex-col">
                        ...
                        <div className="flex items-start gap-5 mb-6">
                          <motion.div 
                            ...
                            transition={{ duration: 0.8 }}
                          >
                            <div className={`p-4 rounded-2xl bg-gradient-to-br ${cert.color} group-hover:scale-110 transition-all duration-500 shadow-xl`}>
                              <CompanyLogo src={cert.logoUrl} alt={cert.issuer} size={40} />
                            </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-slate-900 text-lg mb-3 leading-tight group-hover:text-blue-600 transition-colors duration-500 line-clamp-2">
                              {cert.name}
                            </h4>
                            <div className="flex items-center gap-3 mb-3">
                              <motion.span 
                                className={`text-xs font-bold px-4 py-2 rounded-full bg-gradient-to-r ${cert.color} text-white shadow-lg`}
                                whileHover={{ scale: 1.05 }}
                              >
                                {cert.issuer}
                              </motion.span>
                            </div>
                            <p className="text-slate-500 text-sm font-semibold">{cert.date}</p>
                          </div>
                        </div>
                        ...
                        {/* Enhanced Footer */}
                        <div className="mt-auto pt-6 border-t border-slate-100 group-hover:border-blue-100 transition-colors duration-500">
                          <div className="flex items-center justify-between">
                            ...
                            <motion.a
                              ...
                              className={`inline-flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-bold bg-gradient-to-r ${cert.color} text-white hover:shadow-xl transition-all duration-300 group/btn`}
                              whileHover={{ scale: 1.05, y: -2 }}
                              whileTap={{ scale: 0.95 }}
                            >
                              <span>Verify</span>
                              <ExternalLink size={14} className="group-hover/btn:translate-x-0.5 transition-transform duration-200" />
                            </motion.a>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-500">
                        <motion.div 
                          ...
                          animate={{ 
                            scale: hoveredCard === `cert-${index}` ? 1 : 0, 
                            rotate: hoveredCard === `cert-${index}` ? 0 : -180 
                          }}
